class CNmoney:
    ...
    def cwchange(self, data):
        ...
        if len(cdata) == 1:
            ...
            cski = self.csplit(cki)
            ikl = len(cski)  # 获取拆分后的List长度
            # 大写合并
            for i in range(ikl):
                if self.cschange(cski[i]) == '':  # 有可能一个字符串全是0的情况
                    chk = chk + self.cschange(cski[i]) + self.xdict[ikl - i] # 此时不需要将数字标识符引入
                else:
...
if __name__ == '__main__':
    pt = CNmoney()
    print(pt.cwchange('18000'))
    print(pt.cwchange('30000.0'))
    print(pt.cwchange('30000.00'))
    print(pt.cwchange(19990))
